...
@interface MMMessageToolBar()<UITextViewDelegate,ChatFaceViewDelegate>{
...
@implementation MMMessageToolBar
...
- (void)setupSubviews
{
    ...
    [self.moreButton setImage:[UIImage imageNamed:@"chatBar_moreH"] forState:UIControlStateHighlighted];
    ...
    //表情
    self.faceButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMinX(self.moreButton.frame) - kInputTextViewMinHeight - kHorizontalPadding+4, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
    ...
    [self.faceButton setImage:[UIImage imageNamed:@"chatBar_faceH"] forState:UIControlStateHighlighted];
    ...
    // 输入框的高度和宽度
    CGFloat width = CGRectGetWidth(self.bounds) - (allButtonWidth ? allButtonWidth : (textViewLeftMargin * 2))+3;
    // 初始化输入框
    ...
    //录制
    self.recordButton = [[UIButton alloc] initWithFrame:CGRectMake(textViewLeftMargin, kVerticalPadding-4, width, kInputTextViewMinHeight+8)];
    self.recordButton.titleLabel.font = [UIFont boldSystemFontOfSize:16.0];
    ...
    [self.recordButton setBackgroundImage:[[UIImage imageNamed:@"chatBar_recordBgH"] stretchableImageWithLeftCapWidth:10 topCapHeight:10] forState:UIControlStateHighlighted];
    ...
    if (!self.moreView) {
        self.moreView = [[MMChatBarMoreView  alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, 80) type:ChatMoreTypeGroupChat];
        self.moreView.backgroundColor = [UIColor lightGrayColor];
        self.moreView.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    }
    
    if (!self.faceView) {
        self.faceView = [[MMChatFaceView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, 200)];
        [(MMChatFaceView *)self.faceView setDelegate:self];
        self.faceView.backgroundColor = [UIColor clearColor];
        self.faceView.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    }
    
    if (!self.recordView) {
        self.recordView = [[MMRecordView alloc] initWithFrame:CGRectMake(90, 130, 140, 140)];
    }
...
- (void)faceViewSendFace:(NSString *)faceName{
    if ([faceName isEqualToString:@"[删除]"]) {
        ...
        self.inputTextView.text = [self.inputTextView.text stringByAppendingString:faceName];
        [self textViewDidChange:self.inputTextView];
    }
}
...
{
    if ([self.recordView isKindOfClass:[MMRecordView class]]) {
        [(MMRecordView *)self.recordView recordButtonTouchDown];
    }
...
- (void)recordButtonTouchUpOutside
{
    ...
    if ([self.recordView isKindOfClass:[MMRecordView class]]) {
        [(MMRecordView *)self.recordView recordButtonTouchUpOutside];
    }
...
{
    if ([self.recordView isKindOfClass:[MMRecordView class]]) {
        [(MMRecordView *)self.recordView recordButtonTouchUpInside];
    }
...
{
    if ([self.recordView isKindOfClass:[MMRecordView class]]) {
        [(MMRecordView *)self.recordView recordButtonDragOutside];
    }
...
{
    if ([self.recordView isKindOfClass:[MMRecordView class]]) {
        [(MMRecordView *)self.recordView recordButtonDragInside];
    }
...
- (void)cancelTouchRecord
{
    ...
    if ([_recordView isKindOfClass:[MMRecordView class]]) {
      //  [(MMRecordView *)_recordView recordButtonTouchUpInside];
