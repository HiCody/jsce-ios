...
@interface MMChatFaceView ()<UIScrollViewDelegate>
...
- (void)resetScrollView{
    [self.recentButton setSelected:self.faceViewType == ShowRecentFace];
    [self.emojiButton setSelected:self.faceViewType == ShowEmojiFace];
...
- (void)setupRecentFaces{
    ...
    NSMutableArray *recentFaces = [NSMutableArray arrayWithArray:[FaceManager recentFaces]];
    NSUInteger line = 0;
    NSUInteger column = 0;
...
- (void)setupEmojiFaces{
    ...
    NSMutableArray *allFaces = [NSMutableArray arrayWithArray:[FaceManager emojiFaces]];
...
- (void)handleTap:(UITapGestureRecognizer *)tap{
    NSString *faceName = [FaceManager faceImageNameWithFaceID:tap.view.tag];
    //    NSString *faceName = [XMFaceManager faceNameWithFaceImageName:[NSString stringWithFormat:@"%ld",tap.view.tag]];
    if (tap.view.tag != 999) {
        [FaceManager saveRecentFace:@{@"face_id":[NSString stringWithFormat:@"%ld",tap.view.tag],@"face_name":faceName}];
    }
...
- (void)handleLongPress:(UILongPressGestureRecognizer *)longPress{
    ...
    if (longPress.state == UIGestureRecognizerStateBegan) {
        [self.facePreviewView setCenter:CGPointMake(touchPoint.x, touchPoint.y - 50)];
...
- (NSUInteger)maxPerLine{
    ...
    //iPhone6: 一排8个
    if(iphone6_4_7) rowCount = 7;
    
    //iPhone 6P: 一排10个
    if(iphone6Plus_5_5) rowCount = 9;
...
- (NSUInteger)maxLine{
    if (self.faceViewType == ShowEmojiFace) {
        return 3;
    }else if (self.faceViewType == ShowRecentFace || self.faceViewType == ShowGifFace){
        return 2;
    }
    return 0;
}
