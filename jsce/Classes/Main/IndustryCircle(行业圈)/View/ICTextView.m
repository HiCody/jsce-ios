...
@implementation ICTextView{
    NSString *_oldString;  //为替换表情等得字符串
    NSString *_newString;  //替换过得字符串
...
- (void)cookEmotionString{
    NSArray *itemIndexes  = [RegularExpressionManager itemIndexesWithPattern:EmotionItemPattern inString:_oldString];
...
- (NSAttributedString *)createAttributedEmotionStringWithRanges:(NSArray *)ranges
    ...
    helvetica =CTFontCreateWithName(((CFStringRef) __builtin___CFStringMakeConstantString ("" "Helvetica" "")), FontSize, NULL);
    ...
    UIColor *color=[UIColor hexFloatColor:@"10141a"];
    if (self.fontInteger==1) {
        color= [UIColor hexFloatColor:@"41454b"];
    }
    
    [attrString addAttribute:(id)kCTForegroundColorAttributeName value:(id)color.CGColor range:NSMakeRange(0, [attrString.string length])];
...
CTRunDelegateRef newEmotionRunDelegate(){
    ...
    imageCallbacks.dealloc =ICRunDelegateDeallocCallback;
    imageCallbacks.getAscent=ICRunDelegateGetAscentCallback;
    imageCallbacks.getDescent = ICRunDelegateGetDescentCallback;
    imageCallbacks.getWidth =ICRunDelegateGetWidthCallback;
...
void ICRunDelegateDeallocCallback( void* refCon ){
    // CFRelease(refCon);
}

CGFloat ICRunDelegateGetAscentCallback( void *refCon ){


    return FontHeight;
}

CGFloat ICRunDelegateGetDescentCallback(void *refCon){
    return 0.0;
}

CGFloat ICRunDelegateGetWidthCallback(void *refCon){
